...
import { mediaProcessor } from "@/lib/Tools/MediaProcessor";
import { MediaType } from "@/lib/types/TypesLogic";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isGif && mediaType === "image") {
      ...
      const processedMedia = await mediaProcessor.processMedia(
        buffer,
        mediaType,
        file.name,
        {
    ...
    const mimeType =
      mediaType === "image" ? `image/${finalFormat}` : `video/${finalFormat}`;
